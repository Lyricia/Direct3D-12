...
float4 PSTerrain(VS_TERRAIN_OUTPUT input) : SV_TARGET
{
    ...
    cDetailTexColor = cDetailTexColor * 0.5f;
    //cBaseTexColor = cBaseTexColor * 0.5f;
...
Texture2D gtxtScene : register(t12);

PS_MINIMAP_INPUT VS_Minimap(VS_MINIMAP_INPUT input, uint nVertexID : SV_VertexID) : SV_POSITION
{
...
float4 PS_Minimap(PS_MINIMAP_INPUT input, float4 position : SV_POSITION) : SV_Target
{
    float cColor;
