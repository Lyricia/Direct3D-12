...
{
	m_xmf4x4ToParentTransform._31 = xmf3Look.x;
	m_xmf4x4ToParentTransform._32 = xmf3Look.y;
	m_xmf4x4ToParentTransform._33 = xmf3Look.z;
	XMFLOAT3 right = Vector3::CrossProduct(GetUp(), GetLook());
	...
	for ( ; ; )
	{
		...
		else if (!_tcscmp(pstrToken, _T("EndOfFrame")))
		{
			...
			if ((nNormals > 0) && (nTextureCoords > 0) && (pstrAlbedoTextureName[0] != '\0'))
			{
				...
				_tcscpy_s(pstrPathName, 128, _T("Resource/Model/"));
...
{
	LoadGeometryFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, L"Resource/Model/Apache.txt");
...
{
	LoadGeometryFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, L"Resource/Model/Gunship.txt");
...
{
	LoadGeometryFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, L"Resource/Model/Flyer.txt");
}
...
CHeightMapTerrain::CHeightMapTerrain(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, LPCTSTR pFileName, int nWidth, int nLength, int nBlockWidth, int nBlockLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color) : CGameObject(0)
{
	...
	pTerrainTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Resource/Terrain/4505.dds", 0);
 	pTerrainTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Resource/Terrain/235.dds", 1);
	pTerrainTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Resource/Terrain/235normal.dds", 2);
...
CSkyBox::CSkyBox(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature) : CGameObject(6)
{
	...
	//pSkyBoxTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Resource/SkyBox/ThickCloudsWaterFront.dds", 0);
	//pSkyBoxTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Resource/SkyBox/ThickCloudsWaterBack.dds", 1);
	//pSkyBoxTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Resource/SkyBox/ThickCloudsWaterLeft.dds", 2);
	//pSkyBoxTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Resource/SkyBox/ThickCloudsWaterRight.dds", 3);
	//pSkyBoxTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Resource/SkyBox/ThickCloudsWaterUp.dds", 4);
	//pSkyBoxTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Resource/SkyBox/ThickCloudsWaterDown.dds", 5);
...
void CSkyBox::SetPosition(XMFLOAT3 xmf3Position)
{
	m_xmf4x4World._41 = xmf3Position.x;
	m_xmf4x4World._42 = xmf3Position.y;
	m_xmf4x4World._43 = xmf3Position.z;
